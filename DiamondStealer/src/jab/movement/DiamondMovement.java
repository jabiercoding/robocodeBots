...
public class DiamondMovement extends Movement {
	...
	private void updateFriendFire(){
		...
		while (friendFire.hasMoreElements()){
			...
			if (bul.isFriendFire){
				...
				gravPoint.x = bul.x;
				gravPoint.y = bul.y;
				gravPoint.power = BOT_FORCE*Math.max(0, (1-bul.distance(new Point2D.Double(bot.getX(),bot.getY()))/500));
	...
	private EnemyWave getHitWave(double velocity){
		...
		for (int x = 0; x < enemyWaves.size(); x++) {
			...
			if (Math.abs(ew.travelledDistance
					- new Point2D.Double(bot.getX(),bot.getY()).distance(ew.origin)) < 50
	...
	private void updateBulletHitsBullet(){
		for (int i = 0; i < bulletHitsBullet.size(); i++){
			...
			if (!bot.battleField.contains(new Point2D.Double(bullet.x,bullet.y))){
				bulletHitsBullet.remove(bullet);
			}
	...
	private void updateEnemyWaves(){
		for (int i=0; i<enemyWaves.size(); i++){
			...
			if (hitBy.contains(ew.name) || bot.getOthers()==1)
				createGravLine(ew);
			if (ew.travelledDistance > ew.origin.distance(new Point2D.Double(bot.getX(),bot.getY()))+BOT_WIDTH){
...
//	}
	...
	private void createGravLine(EnemyWave ew){
		...
		while (bot.battleField.contains(point)){
			...
			if (ew.travelledDistance < ew.origin.distance(new Point2D.Double(bot.getX(),bot.getY()))){
...
//				double distancePointToMe = point.distance(new Point2D.Double(bot.getX(),bot.getY()));
//				double forcePerOne = 1-Math.min(1,distancePointToMe/MAX_DISTANCE_LINES);
//				double distanceWaveToMe = ew.origin.distance(new Point2D.Double(bot.getX(),bot.getY()))-ew.travelledDistance;
//				double modificator = 1-Math.min(1,distanceWaveToMe/MAX_DISTANCE_VAR_SEGM);
//				gp.power= MAX_LINE_FORCE*modificator*forcePerOne;
//				gravPoints.add(gp);
			}
			point.setLocation(point.x+Math.sin(ew.lineAngle)*LINE_POINTS_DISTANCE,point.y+Math.cos(ew.lineAngle)*LINE_POINTS_DISTANCE);
		}
		if (closestGP != null){
			double distancePointToMe = closestGP.distance(new Point2D.Double(bot.getX(),bot.getY()));
			double forcePerOne = 1-Math.min(1,distancePointToMe/MAX_DISTANCE_LINES);
			double distanceWaveToMe = ew.origin.distance(new Point2D.Double(bot.getX(),bot.getY()))-ew.travelledDistance;
			double modificator = 1-Math.min(1,distanceWaveToMe/MAX_DISTANCE_VAR_SEGM);
	...
	public void listen(Event e){
		...
		if (e instanceof ScannedRobotEvent){
			...
			if (energyDrop>0 && energyDrop<=3){
				int var1_Distance = (int) ((Math.min(MAX_DISTANCE_VAR_SEGM, bot.enemy.distance) / (MAX_DISTANCE_VAR_SEGM/(VARIABLE_BINS-1))));
				...
				double lineAngle = relativeAngle + absBearing(enemy.x,enemy.y,bot.getX(),bot.getY());
				enemyWaves.add(new EnemyWave(enemy.name,new Point2D.Double(enemy.x,enemy.y),robocode.Rules.getBulletSpeed(energyDrop),new Point2D.Double(bot.getX(),bot.getY()),vars,lineAngle));		
			}
			}
		}
	...
	private void logHit(EnemyWave wave, Point2D.Double myPosition){
		...
		.atan2(wave.me.x - wave.origin.x, wave.me.y
				- wave.origin.y));
		double thetaBreak = Utils.normalAbsoluteAngle(Math.atan2(
		bot.getX() - wave.origin.x, bot.getY()
				- wave.origin.y));
	...
	private void calculateForceAndMove() {
		...
		while (others.hasMoreElements()){
			...
			if (!botInfo.name.equals(bot.getName())){
				...
				if(bot.enemy!=null && bot.enemy.name.equals(botInfo.name)){
					//botGravPoint.power = (bot.enemy.distance<=250)?BOT_FORCE*2:-BOT_FORCE*5;
					botGravPoint.power = /*10**/ 5*BOT_FORCE*(1-bot.enemy.distance/ENEMY_DISTANCE);
				}else{
		 ...
		 */
		xforce += (-WALL_FORCE) / Math.pow(getRange(bot.getX(), bot.getY(), bot
				.getBattleFieldWidth(), bot.getY()), 2);
		xforce -= (-WALL_FORCE) / Math.pow(getRange(bot.getX(), bot.getY(), 0, bot
				.getY()), 2);
		yforce += (-WALL_FORCE) / Math.pow(getRange(bot.getX(), bot.getY(), bot.getX(),
...
//			}
			if (hitBy.contains(ew.name))
				g.drawLine((int)ew.origin.x, (int)ew.origin.y, (int)(ew.origin.x+Math.sin(ew.lineAngle)*1000), (int)(ew.origin.y+Math.cos(ew.lineAngle)*1000));
			//g.draw(ew);
		}
		...
		for (int i=0;i<gravPoints.size();i++){
			...
			if (size<=1)
				g.fillOval((int) (point.getX() - 16*size), (int) (point.getY() - 16*size), (int)(32*size), (int)(32*size));
			else
				g.drawOval((int) (point.getX() - 16*size), (int) (point.getY() - 16*size), (int)(32*size), (int)(32*size));
		}
		...
		g.drawLine((int)bot.getX(), (int)bot.getY(), (int)(10*-xforce + bot.getX()), (int)(10*-yforce + bot.getY()));
